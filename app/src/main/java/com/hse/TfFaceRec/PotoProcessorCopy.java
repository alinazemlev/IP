...
/**
 * Created by avsavchenko original class PhotoProcessor.
 * PhotoProcessorCopy my bad overrite
 */
public class PhotoProcessorCopy{
    /**
     * Tag for the {@link Log}.
     */
    private static final String TAG = "PhotoProcessorCopy";

    private ArrayList<TensoflowWrapper> facesClassifiers;

    private static int countModels = 4;
    ...
    private static PhotoProcessorCopy instance;
    ...
    private Map<String,String> labels2HighLevelCategories = new HashMap<>();


    private PhotoProcessorCopy(final Activity context, AssetManager assetManager) {
    ...
    public PhotoProcessorCopy(ArrayList<TensoflowWrapper> facesClassifiers, Context context) {
        this.facesClassifiers = facesClassifiers;
        this.context = context;
    }

    public PhotoProcessorCopy(Activity context) {

    }

    public static PhotoProcessorCopy getPhotoProcessor(final Activity context) {
        if (instance == null) {
            instance = new PhotoProcessorCopy(context);
        }
        return instance;
    }
    ...
    private void loadModels(AssetManager assetManager){
        for (int i =0; i < countModels; ++i){
            TensoflowWrapper wrap = new TensoflowWrapper();
            if (i==0){
                wrap.initTensorFlowAndLoadModel(assetManager);
            }
            facesClassifiers.add(wrap);

        }
        Log.i(TAG, "Models are loaded");
    }
    ...
    private static <V> void save(Context context, String filename, ConcurrentHashMap<String, V> map, String key, V val) {
        try {
            ...
            map.put(key, val);
            ObjectOutputStream os = new PhotoProcessorCopy.AppendingObjectOutputStream(context.openFileOutput(filename, Context.MODE_APPEND));
            os.writeObject(key);
            os.writeObject(val);
            os.close();
        } catch (Exception e) {
    ...
    private synchronized SceneData classifyImage(Bitmap bmp, StringBuilder text) {
        long startTime = SystemClock.uptimeMillis();

        Bitmap scenesBitmap = Bitmap.createScaledBitmap(bmp, facesClassifiers.get(0).getInputSize(),
                facesClassifiers.get(0).getInputSize(), false);
        facesClassifiers.get(0).classifyImage(scenesBitmap);
    ...
    private void initPhotosTaken() {
        ...
        //String path= android.os.Environment.getExternalStorageDirectory().toString()+"/Pic/Camera";
